...
var _origOpen=window.openModal;if(typeof _origOpen==='function'){window.openModal=function(id){var el=document.getElementById(id);if(el)el.style.display='';_origOpen(id);setTimeout(function(){fixModalFooter();fixModalScroll();if(id==='modal-addSupplier')fixSupplierLayout();if(id==='modal-addPR')fixPRRequester();if(id==='modal-addSupInv')_injectScanBtn('modal-addSupInv','scan-supinv-btn','สแกนใบแจ้งหนี้',window.scanSupInv);},80);};}
...
window.scanExpense=function(){_scanFile('scan-expense-btn','expense',function(d){if(d.invoice_date)document.getElementById('exp-date').value=d.invoice_date;if(d.supplier_name)document.getElementById('exp-vendor').value=d.supplier_name;if(d.supplier_address){var elAddr=document.getElementById('exp2-vendor-addr');if(elAddr)elAddr.value=d.supplier_address;}if(d.supplier_tax_id){var elTax=document.getElementById('exp2-vendor-taxid');if(elTax)elTax.value=d.supplier_tax_id;}if(d.job_name)document.getElementById('exp-job').value=d.job_name;if(d.note)document.getElementById('exp-note').value=d.note;if(d.wht_rate!=null)document.getElementById('exp-wht-rate').value=d.wht_rate;if(d.items&&d.items.length>0){var elItems=document.getElementById('exp2-items-data');if(elItems){elItems.value=JSON.stringify(d.items.map(function(it){return{desc:it.item_name||'',cat:'อื่นๆ',qty:it.qty||1,price:it.unit_price||0};}));if(typeof renderExpenseLineItems==='function')renderExpenseLineItems();}}toast('อ่านเรียบร้อย','success');});};
window.scanReceive=function(){_scanFile('scan-receive-btn','receive',function(d){var items=d.items&&d.items.length>0?d.items:[d];var fill=function(item){if(item.qty!=null)document.getElementById('recv-qty').value=item.qty;if(item.cost!=null)document.getElementById('recv-cost').value=item.cost;if(item.lot_number)document.getElementById('recv-lot').value=item.lot_number;if(item.mfg_date)document.getElementById('recv-mfg-date').value=item.mfg_date;if(item.expiry_date)document.getElementById('recv-expiry').value=item.expiry_date;if(d.receive_date||item.receive_date)document.getElementById('recv-date').value=d.receive_date||item.receive_date;if(d.supplier_name||item.supplier_name)document.getElementById('recv-supplier').value=d.supplier_name||item.supplier_name||'';if(d.po_number||item.po_number)document.getElementById('recv-po').value=d.po_number||item.po_number||'';var sel=document.getElementById('recv-item');if(sel&&(item.item_name||d.item_name)){var name=item.item_name||d.item_name;var opts=Array.from(sel.options).map(function(o){return{v:o.value,t:o.text};});var nameLo=name.toLowerCase();var best=opts.find(function(o){return o.t.toLowerCase().includes(nameLo)||nameLo.includes(o.t.toLowerCase().split(' ')[0]);});if(best)sel.value=best.v;}};if(items.length===1){fill(items[0]);toast('อ่านเรียบร้อย','success');}else{_showPickerModal('📦 เลือกรายการ',items,function(item,i,onChange){var row=document.createElement('div');row.style.cssText='display:flex;align-items:flex-start;gap:10px;padding:10px;background:var(--surface2,#f5f5f5);border-radius:8px;';var cb=document.createElement('input');cb.type='checkbox';cb.checked=true;cb.style.marginTop='3px';cb.onchange=function(){onChange(cb.checked);};var info=document.createElement('div');info.style.flex='1';info.innerHTML='<div style="font-weight:600;">'+(item.item_name||'-')+'</div><div style="font-size:12px;">จำนวน: '+(item.qty||'-')+'</div>'+(item.expiry_date?'<div style="font-size:12px;color:var(--warning);">วนหมด: '+item.expiry_date+'</div>':'');row.appendChild(cb);row.appendChild(info);return row;},function(selected){if(!selected.length)return;fill(selected[0]);toast('เติมเรียบร้อย','success');});}});};
    ...
    Promise.all([
      ...
      if (dD.length > 0) {
        ...
        dD.forEach(function(x) {
          ...
          d.querySelector('[data-diet-del]').addEventListener('click', async function(){
      ...
      if (tD.length === 0) {
        ...
        tDshow.forEach(function(x) {
          ...
          d.querySelector('[data-tube-del]').addEventListener('click', async function(){
...
// ===== EXPENSE =====
// Phase 4 Step E commit 7: ลบ override ของ modal-expense (เก่า) — ลบใน commit 6
// ของใหม่ (openAddExpenseModal ใน expenses.js) มี reset logic + default values ใน-built อยู่แล้ว
