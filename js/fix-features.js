...
var _origOpenAddExp=window.openAddExpenseModal;if(typeof _origOpenAddExp==='function'){window.openAddExpenseModal=function(){_origOpenAddExp.apply(this,arguments);setTimeout(function(){if(document.getElementById('scan-expense-btn'))return;var modal=document.getElementById('modal-addExpense');if(!modal)return;var closeBtn=modal.querySelector('.modal-close');if(!closeBtn)return;var btn=document.createElement('button');btn.id='scan-expense-btn';btn.className='btn btn-ghost btn-sm';btn.style.cssText='font-size:13px;display:inline-flex;align-items:center;margin-right:8px;';btn.innerHTML='📷 สแกน';btn.onclick=function(){scanExpense();};closeBtn.parentNode.insertBefore(btn,closeBtn);},80);};}
