...
async function renderPhysioPackagesTab() {
  ...
  var _ref = await supa.from('physio_packages').select('*, patients(name)').eq('is_active', true).order('created_at', { ascending: false });
  var packages = _ref.data || [];
  var err = _ref.error;

  if (err) { container.innerHTML = '<div style="color:#e74c3c;padding:20px;">โหลดข้อมูลไม่สำเร็จ: '+err.message+'</div>'; return; }
  ...
  container.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">' +
    ...
    '<div style="font-size:22px;font-weight:700;color:var(--accent);">' + packages.length + '</div></div>' +
    ...
    '<div style="font-size:22px;font-weight:700;">' + totalSessions + '</div></div>' +
    '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;">' +
    '<div style="font-size:11px;color:var(--text3);">✅ ใช้ไปแล้ว</div>' +
    '<div style="font-size:22px;font-weight:700;color:var(--warning);">' + totalUsed + '</div></div>' +
    '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;">' +
    '<div style="font-size:11px;color:var(--text3);">⏳ คงเหลือ</div>' +
    '<div style="font-size:22px;font-weight:700;color:var(--accent);">' + (totalSessions - totalUsed) + '</div></div>' +
    '</div>' +
    (!packages.length ? '<div style="text-align:center;padding:40px;color:var(--text3);">ยังไม่มี Package กายภาพ</div>' :
    '<div style="overflow-x:auto;">' +
    '<table class="data-table"><thead><tr>' +
    '<th>ผู้รับบริการ</th><th>ชื่อ Package</th>' +
    '<th style="text-align:center;">Session</th><th style="text-align:center;">ใช้แล้ว</th>' +
    '<th style="text-align:center;">คงเหลือ</th><th>ราคา/ชม. เกิน</th>' +
    '<th>วันเริ่ม</th><th>วันหมด</th><th></th>' +
    '</tr></thead><tbody>' +
    packages.map(function(p) {
      var rem = p.sessions_included - p.sessions_used;
      var pct = p.sessions_included > 0 ? Math.round((p.sessions_used/p.sessions_included)*100) : 0;
      var bc = pct >= 100 ? '#e74c3c' : pct >= 75 ? '#e67e22' : 'var(--accent)';
      return '<tr>' +
        '<td style="font-weight:600;">' + (p.patients && p.patients.name ? p.patients.name : '-') + '</td>' +
        '<td>' + p.name + '</td>' +
        '<td style="text-align:center;">' + p.sessions_included + '</td>' +
        '<td style="text-align:center;"><div>' + p.sessions_used + '</div>' +
        '<div style="width:60px;height:4px;background:var(--border);border-radius:2px;margin:3px auto 0;">' +
        '<div style="width:' + Math.min(pct,100) + '%;height:100%;background:' + bc + ';border-radius:2px;"></div></div></td>' +
        '<td style="text-align:center;font-weight:700;color:' + (rem<=0?'#e74c3c':'var(--accent)') + ';">' + rem + '</td>' +
        '<td>' + (rem <= 0 ? '-' : formatThb(p.rate_per_hour_extra||0)+'/ชม.') + '</td>' +
...
function openAddPhysioPackageModal(editId) {
  ...
  var html = '<div class="modal-overlay" id="modal-physio-pkg" style="display:flex;">' +
    '<div class="modal" style="max-width:480px;width:95%;">' +
    '<div class="modal-header"><div class="modal-title">🤸 ' + (editId ? 'แก้ไข' : 'เพิ่ม') + ' Package กายภาพ</div>' +
    '<button class="modal-close" onclick="closeModal('modal-physio-pkg')">✕</button></div>' +
    '<div class="modal-body">' +
    '<input type="hidden" id="physio-pkg-edit-id" value="' + (editId||'') + '">' +
    '<div class="form-group"><label>ผู้รับบริการ <span class="required">*</span></label>' +
    '<select id="physio-pkg-patient" class="form-control"><option value="">-- เลือก --</option>' +
    patients.map(function(p){ return '<option value="'+p.id+'">'+p.name+'</option>'; }).join('') +
    '</select></div>' +
    '<div class="form-group"><label>ชื่อ Package <span class="required">*</span></label>' +
    '<input id="physio-pkg-name" class="form-control" placeholder="เช่น กายภาพ 10 ครั้ง"></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
    '<div class="form-group"><label>จำนวน Session</label>' +
    '<input type="number" id="physio-pkg-sessions" class="form-control number" min="1" value="10"></div>' +
    '<div class="form-group"><label>ราคา/ชม. ที่เกิน (฿)</label>' +
    '<input type="number" id="physio-pkg-rate" class="form-control number" min="0" value="0"></div></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
    '<div class="form-group"><label>วันเริ่ม</label>' +
    '<input type="date" id="physio-pkg-start" class="form-control" value="' + new Date().toISOString().split('T')[0] + '"></div>' +
    '<div class="form-group"><label>วันหมดอายุ (ถ้ามี)</label>' +
    '<input type="date" id="physio-pkg-end" class="form-control"></div></div>' +
    '<div class="form-group"><label>หมายเหตุ</label>' +
    '<input id="physio-pkg-note" class="form-control" placeholder="ไม่จำเป็น"></div>' +
    '</div>' +
...
async function openEditPhysioPackage(id) {
  var _r = await supa.from('physio_packages').select('*').eq('id', id).single();
  var pkg = _r.data;
...
async function savePhysioPackage() {
  ...
  var data = { patient_id: patientId, name: name, sessions_included: sessions, rate_per_hour_extra: rate, start_date: startDate, end_date: endDate, is_active: true, note: note, created_by: currentUser && (currentUser.name || currentUser.username) || '' };
  var _res = editId ? await supa.from('physio_packages').update(data).eq('id', editId) : await supa.from('physio_packages').insert(Object.assign({sessions_used:0}, data));
  if (_res.error) { toast('บันทึกไม่สำเร็จ: '+_res.error.message, 'error'); return; }
...
async function deactivatePhysioPackage(id) {
  ...
  var _r = await supa.from('physio_packages').update({ is_active: false }).eq('id', id);
  if (_r.error) { toast('ไม่สำเร็จ: '+_r.error.message, 'error'); return; }
