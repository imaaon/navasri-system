...
<div class="tab" onclick="switchPatTab('dispense')">💊 เบิกสินค้า</div>
  ...
  document.getElementById('patprofile-tab-meds').innerHTML = (function(){
    ...
    const rows = meds.map(m => '<tr><td style="font-weight:600;">' + m.name + '</td><td style="font-size:12px;color:var(--text2);">' + (m.note||'-') + '</td><td style="font-size:12px;">' + (m.createdBy||actor||'-') + '</td><td style="display:flex;gap:4px;"><button class="btn btn-ghost btn-sm" onclick="deleteMedication(\'' + pid + '\',\'' + m.id + '\')">🗑️</button></td></tr>').join('');
    return '<div class="card"><div class="card-header"><div class="card-title" style="font-size:13px;">💊 ยาประจำ (' + meds.length + ' รายการ)</div><button class="btn btn-primary btn-sm" onclick="openAddMedModal(\'' + pid + '\')">+ เพิ่มยา</button></div><div class="table-wrap"><table><thead><tr><th>ชื่อยา</th><th>หมายเหตุ</th><th>ผู้กรอก</th><th></th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
  })();
