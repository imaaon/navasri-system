...
async function renderPhysioTab(patientId) {
  ...
  summaryEl.innerHTML = "<div style=\"text-align:center;\"><div style=\"font-size:11px;color:var(--text3);\">จำนวน Session</div><div style=\"font-size:20px;font-weight:700;color:var(--accent);\">" + (sessions||[]).length + "</div></div><div style=\"text-align:center;\"><div style=\"font-size:11px;color:var(--text3);\">ยังไม่รวมบิล</div><div style=\"font-size:20px;font-weight:700;color:var(--warning);\">" + unbilledAmt.toLocaleString("th-TH",{minimumFractionDigits:0}) + " ฿</div></div><div style=\"text-align:center;\"><div style=\"font-size:11px;color:var(--text3);\">รวมในบิล</div><div style=\"font-size:20px;font-weight:700;color:var(--success);\">" + billedAmt.toLocaleString("th-TH",{minimumFractionDigits:0}) + " ฿</div></div>";
  var p = db.patients.find(function(x) { return String(x.id) === String(patientId); });
  ...
  var rows = (sessions||[]).map(function(s) {
    ...
    var editBtn = !s.billed
      ? "<button class=\"btn btn-ghost btn-xs\" onclick=\"openPhysioSessionModal('" + patientId + "','" + patName + "','" + s.id + "')\">✏️</button> <button class=\"btn btn-ghost btn-xs\" style=\"color:var(--danger-text);\" onclick=\"deletePhysioSession('" + s.id + "','" + patientId + "')\">🗑️</button>"
